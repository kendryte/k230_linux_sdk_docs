...
您可以在部署过程中，按需更换模型、任务类型、任务模式、阈值参数等，其中**标签文本文件中，每个标签独占一行**。

## 注意事项

- 目前支持的模型为 `yolov5`、`yolov8` 和 `yolo11`。
- 目前支持的任务类型为 `yolov5`支持`classify`、`detect` 和 `segment`三种任务,`yolov8`和`yolo11`支持 `classify`、`detect`、`segment`和`obb`四种任务。
